...
A parameters file called "Parameters.xlsx" must be in the same folder as the PIXEorg executable. This Parameters excel file has: <br/><br/>
  ...
  -	a sheet called "Compounds" where the user indicates in a table the elements and oxides, along with the stoichiometric coefficents, whose mass concentrations are required. The format of each oxide entry must be "Element 1, Coeficient 1, Element 2, Coeficient 2, Element 3, ...". An unlimited number of elements in each oxide can be considered. All of the elements that consitute all of the elements/oxides marked with "Y" in the first column ("Include in stoichiometry?") are included in the stoichiometry presented for each of the samples analyzed. In this way the user can obtain the stoichiometry of a particular compound present in a sample (or samples). <br/><br/>
