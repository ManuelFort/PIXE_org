...
Finally, in the last sheet called "Stoichiometry", there is a table with the stoichiometry, for each sample, of the elements present in each of the oxides marked with "Y" in the first column ("Include in stoichiometry?") of the Compounds sheet in the Parameters.xlsx file. Only elements with avaliable lines in the csv data files are considered. Each stoichiometric coefficient will have 2 significant digits except for the case where there are trailing zeros after the decimal separator (Excel does not show them).
...
The stoichiometry table presents the stoichiometry of all of the elements present in all of the oxides marked with "Y" in the first column ("Include in stoichiometry?") of the Compounds sheet in the "Parameters.xlsx" file. Only elements with avaliable lines in the csv data files are considered. The line concentrations used for each element are exactly the same as the ones used to obtain the concentration of the respective compound, the only difference being in that to present the concentration of the compound the line concentration is multiplied by the appropriate mass factor, and to obtain the stoichiometry coefficient the line concentration is divided by the atomic mass of the element and this value is then normalized to the highest stoichiometric coefficent for each sample. If there is more than 1 line avaliable for a particular element the lines used for the stoichiometry are the same as the ones choosed to calculate the compound's concentration (see 2.3.1 "Selection of Lines").
