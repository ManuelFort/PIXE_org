...
A parameters file called "Parameters.xlsx" must be in the same folder as the PIXE_org executable. This Parameters excel file has: <br/><br/>
  ...
  -	a sheet called "Compounds" where the user indicates in a table the elements and oxides whose mass concentrations are required. The format of each oxide entry must be "Element 1, Stoichometric Coeficient 1, Element 2, Stoichiometric Coeficient 2, Element 3, ...". An unlimited number of elements in each oxide can be considered. Besides the concentrations, the stoichiometries of the samples can also be presented. All of the elements that consitute all of the elements/oxides marked with "Y" in the first column ("Include in stoichiometry?") are included in the stoichiometry presented for each of the samples analyzed. In this way the user can obtain the stoichiometry of a particular compound present in a sample (or samples). <br/><br/>
