...
### 1.1 Installation
The installer file (setup_pixe_org.exe) can be used to install PIXE_org in any desired folder. The parameters file “Parameters.xlsx” and the the csv data files should be placed in any other folder that the program can write into (a folder that doesn’t require administrator permissions such as the ones in “C:”). 
</br>
</br>
</br>
</br>
...
A parameters file called "Parameters.xlsx" must be in the same folder as the csv data files. If the high energy and low energy data files are in different folders there must be one "Parameters.xlsx" file in one of the folders. This Parameters excel file has: <br/><br/>
  ...
  -	a sheet called "Compounds" where the user indicates in a table the elements and oxides whose mass concentrations are required. Each element/oxide chemical formula is a row of this table. An unlimited number of elements in each oxide can be considered. All the elements (with lines available in the csv data files) that constitute all of the elements/oxides marked with "Y" in the first column ("Include in stoichiometry?") are also included in a stoichiometry table where the stoichiometry of these chosen elements is presented for each of the samples analyzed. In this way the user can obtain the stoichiometry of a particular compound present in a sample (or samples). <br/><br/>
...
The "FinalTable" and "FinalTable Errors" tables have the number of decimal places of each concentration determined by (the same as) its error: one significant digit in the error if the error is <1 or no decimal part if the error is >=1. However if the data is to be further processed it is useful to have a greater precision in the values. Therefore a third sheet, named "FinalTable Extended", has a table equal to the "FinalTable" table but all the entries have 4 decimal places. Since these entries are in "number" format (and the ones in the previous tables are in "string" format) Excel hides the trailing zeros after the decimal separator, which may be present after rounding, and therefore the number of decimal places displayed may be less than 4. For example, if a concentration has the value 33.34599% it is rounded to 33.3460% and displayed as 33.346%.

Finally, the stoichiometry table ("Stoichiometry" sheet) presents the stoichiometry of the chosen elements, for each sample. To select an element just put a "Y" in the first column ("Include in stoichiometry?") of the table in the Compounds sheet of the "Parameters.xlsx" file, next to the element or respective oxide entry. If an oxide is selected note that only the element of this oxide with line measurements avaliable in the csv data files is included in the stoichiometries (oxygen, for example, is not). For each sample, the coefficients are normalized to the greater constituent (of the selected elements). Each stoichiometric coefficient is presented in "number" format with 2 significant digits except for the case where there are trailing zeros after the decimal separator (Excel does not show them).
